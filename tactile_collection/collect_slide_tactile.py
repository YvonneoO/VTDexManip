...
Lever Sliding trajectories using the tactile-scale-fixed slide-t_scr_gt checkpoint
(model_6100.pt, 86.5% success rate, beats base's 78.4% -- see the artifact's
"Tactile-scale fix" section).

Direct adaptation of collect_bottle_cap_tactile.py -- verified 2026-09-07 that
Lever Sliding shares the same base class (tv_tasks.tasks.base.shadow_hand.ShadowHandBase)
as BottleCap, so "hand"/"object" actor names, compute_pixel_obs/img_buf, and
camera_eye_list/camera_lookat_list all carry over unchanged. The one real
difference: slide.py never aliases `self.dof_pos` the way bottle_cap.py does
(`self.dof_pos = self.shadow_hand_dof_pos`) -- it reads `self.shadow_hand_dof_pos`
directly everywhere, so this script does too (see `task.shadow_hand_dof_pos` below;
using `task.dof_pos` here would AttributeError).

Lever Sliding is single-handed, 5 objects in env_dict (ClampingHanger, DeadboltLock,
GasSpring, Padlock, TelescopingSlide) -- --test mode hardcodes numEnvs to
len(env_dict)=5 (hydra_utils.py:624, a global --test behavior, not BottleCap-specific
despite the original script's docstring implying otherwise). This script uses all 5
in parallel, one EgoTouchTaxelMapper + one episode buffer per env, same pattern as
BottleCap's 10.

Output convention identical to collect_bottle_cap_tactile.py / bidexhands'
rollout_tactile_rgb_chest.py: one directory per successful episode under
<out_dir>/successful_episodes/episode_NNNNNN/ containing rgb_frames/frame_%06d.png,
pressure_grids.npz, and trajectory_env0.npz.

Usage:
    python tactile_collection/collect_slide_tactile.py --task slide-t_scr_gt \
        --rl_device cuda:0 --resume_model runs/Sliding/slide/slide-t_scr_gt/seed111/checkpoint/model_6100.pt \
        --test --seed 111 --headless \
...
def main():
    ...
    out_dir = os.environ.get("OUT_DIR", "collected_slide_tactile")
    ...
    while total_successes < target_successes and step < max_steps:
        ...
        for i, env_ptr in enumerate(task.envs):
            ...
            # by 255 again here would blow every value past 255 and clip to solid white
            # (bit BottleCap's collector once, caught via a blank-white RGB check video).
            ...
            # exist on this task; using it here would AttributeError.
